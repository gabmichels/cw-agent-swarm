...
describe('EnhancedMemoryService Integration Tests', () => {
  ...
  describe('Multi-Agent Communication Features', () => {
    it('should store agent communication fields at top level', async () => {
      ...
      const point = points[0];
      expect(point.senderAgentId).toBeDefined();
      expect(point.receiverAgentId).toBeDefined();
      expect(point.communicationType).toBe('task_delegation');
  ...
  describe('Performance Optimizations', () => {
    ...
    it('should handle complex nested metadata extraction', async () => {
      ...
      expect(point.threadId).toBe(threadId);
      expect(point.messageType).toBe('system_notification');
    });
  });
  ...
  describe('Type Safety', () => {
    it('should maintain strict typing throughout operations', async () => {
      ...
      expect(typeof point.userId).toBe('string');
