...
import { MemoryErrorCode } from '../../config';
...
export class EnhancedMemoryService extends MemoryService {
  ...
  async addMemory<T extends BaseMemorySchema>(params: AddMemoryParams<T>): Promise<MemoryResult> {
    try {
      ...
      // CRITICAL FIX: Keep Qdrant-compatible structure
      // Only store id, vector, and payload at top level
      // Move enhanced fields into payload for proper storage
      const qdrantCompatiblePoint: MemoryPoint<T> = {
        id: point.id,
        vector: point.vector,
        payload: {
          ...point.payload,
          // Add indexable fields to payload for storage
          _indexableFields: {
            ...indexableFields,
            timestamp: this.getTimestampFn() // Add numeric timestamp for filtering
          }
        } as unknown as T
      };
    ...
    try {
      ...
      if (searchVector) {
        ...
        results = searchResults.map(result => ({
          ...
          // Add enhanced fields from stored _indexableFields or extract from metadata
          ...((result.payload as any)?._indexableFields || this.extractIndexableFields(result.payload.metadata || {}))
        ...
        results = points.map(point => ({
          ...point,
          // Add enhanced fields from stored _indexableFields or extract from metadata
          ...((point.payload as any)?._indexableFields || this.extractIndexableFields(point.payload.metadata || {}))
        } as EnhancedMemoryPoint<T>));
      } else {
