...
// Optional global cache for task list
declare global {
  var schedulerTasksCache: any[] | undefined;
}
...
export async function GET() {
  try {
    ...
    // Check if we have a global cache first (populated by debug-scheduler endpoint)
    if (globalThis.schedulerTasksCache && Array.isArray(globalThis.schedulerTasksCache) && globalThis.schedulerTasksCache.length > 0) {
      console.log(`Using global cache with ${globalThis.schedulerTasksCache.length} tasks`);
      return NextResponse.json(globalThis.schedulerTasksCache);
    }
    ...
    // If we still have no tasks after all attempts, return default tasks for UI
    if (formattedTasks.length === 0) {
      console.log('No tasks found after all attempts, returning all default tasks');
      ...
      const defaultTasks = [
        ...
        {
          id: TASK_IDS.NEWS_SCAN,
          name: 'News Scan',
          description: 'Monitor marketing-related news and industry updates',
        ...
        {
          id: TASK_IDS.TRENDING_TOPIC_RESEARCH,
          name: 'Trending Topic Research',
          description: 'Research emerging trends in marketing and customer experience',
        ...
        {
          id: TASK_IDS.SOCIAL_MEDIA_TRENDS,
          name: 'Social Media Trends',
          description: 'Monitor trending topics on social media platforms',
        ...
        {
          id: TASK_IDS.MONTHLY_STRATEGIC_PLANNING,
          name: 'Monthly Strategic Planning',
          description: 'Develop strategic marketing plan for the upcoming month',
        ...
        {
          id: TASK_IDS.QUARTERLY_PERFORMANCE_REVIEW,
          name: 'Quarterly Performance Review',
          description: 'Analyze quarterly marketing performance and KPIs',
          cronExpression: '0 10 1 1,4,7,10 *',
          enabled: true,
          lastRun: new Date(Date.now() - 1000 * 60 * 60 * 24 * 30).toISOString(),
          nextRun: new Date(Date.now() + 1000 * 60 * 60 * 24 * 60).toISOString()
        }
      ];
      
      // Save these default tasks to global cache
      try {
        globalThis.schedulerTasksCache = defaultTasks;
        console.log('Updated global cache with default tasks');
      } catch (cacheErr) {
        console.error('Error updating cache with defaults:', cacheErr);
      }
