...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const recentMessagesData = recentMessages.map(result => ({
        id: result.point.id,
      ...
      if (recentMessagesData && recentMessagesData.length > 0) {
        const messageTypes = new Set(recentMessagesData.map(msg => msg.type));
    ...
    for (const memoryType of memoryTypes) {
      ...
      try {
        ...
        const collectionMemories = searchResults.map(result => result.point);
