...
export async function GET() {
  try {
    ...
    const suspectedReflections = allMessages.filter((msg: any) => {
    ...
    if (suspectedReflections.length > 0) {
      ...
      const deletePromises = suspectedReflections.map(async (reflection: any) => {
        try {
    ...
    return NextResponse.json({
      ...
      suspectedMessages: suspectedReflections.map((msg: any) => ({
        id: msg.id,
        text: msg.text,
