/**
 * Simple JavaScript Markdown Memory Test Script
 * 
 * This script tests the retrieval of markdown documents from the memory system.
 * It's a simplified JS version that doesn't require TypeScript compilation.
 */

// Import memory services using correct paths
const { getMemoryServices } = require('../server/memory/services');
const { MemoryType } = require('../server/memory/config/types');
 ...
 */
async function main() {
  console.log('=== MARKDOWN MEMORY TEST (JS VERSION) ===');
  ...
  try {
    ...
    console.error('Error running markdown test:', error);
    process.exit(1);
  }
}
 ...
 */
function analyzeMarkdownDocuments(docs) {
 ...
 */
async function testAddMarkdownDocument(memoryService) {
...
## Introduction
This is a test markdown document created by the markdown-test script.
...
`;
    ...
    const result = await memoryService.addMemory({
      ...
      metadata: {
        ...
        extractedFrom: "markdown-test.js",
    ...
    if (result.success) {
      ...
      global.testMarkdownId = result.id;
    } else {
 ...
 */
async function performCleanup(memoryService) {
  console.log('=== CLEANUP ===');
  ...
  try {
    ...
    for (const doc of testDocuments) {
      if (doc.text?.includes('created by the markdown-test script')) {
