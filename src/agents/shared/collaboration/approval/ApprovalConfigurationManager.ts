...
import { ApprovalRule, ApprovalLevel, ApprovalHistoryEntry, ApprovalHistoryFilter, CollaborativeTask } from '../interfaces/HumanCollaboration.interface';
...
export class ApprovalConfigurationManager {
  ...
  private initializeDefaultRules(): void {
    ...
    this.addRule({
      ...
      condition: (task: CollaborativeTask) => task.type === 'external_post',
      priority: 100,
      enabled: true,
      createdAt: now,
      updatedAt: now,
    ...
    this.addRule({
      ...
      condition: (task: CollaborativeTask) => task.isStrategic === true,
      priority: 90,
      enabled: true,
      createdAt: now,
      updatedAt: now,
    ...
    this.addRule({
      ...
      condition: (task: CollaborativeTask) => {
        const toolName = task.toolName || (task.params?.toolName as string);
    ...
    this.addRule({
      ...
      condition: (task: CollaborativeTask) => {
        // Check if subgoals exist and have high priority
        const hasHighPrioritySubgoals = task.subGoals?.some((sg: any) => sg.priority >= 8) || false;
        // Or check in params for priority score
        const priorityScore = task.params?.priorityScore as number;
    ...
    this.addRule({
      ...
      condition: (task: CollaborativeTask) => {
        ...
        return sensitiveKeywords.some(keyword => 
          ...
          (task.subGoals || []).some((sg: any) => sg.description?.toLowerCase().includes(keyword))
        );
      },
      priority: 95,
      enabled: true,
   ...
   */
  public checkApprovalRequired(task: CollaborativeTask): { required: boolean; rule?: ApprovalRule } {
