...
export default function AgentsDashboard() {
  ...
  return (
    <div className="p-8">
      ...
      {/* 🛡️ Ethics Section */}
      <section className="mb-12 border rounded-lg shadow-sm overflow-hidden">
        <div className="bg-gray-50 p-4 border-b">
          <h2 className="text-xl font-semibold">⚖️ Ethics Violations</h2>
        ...
        {loading ? (
            ...
            <table className="text-sm w-full">
              ...
              <tbody>
                {violations.map((v, idx) => (
                  <tr key={idx} className={`border-t ${v.metadata?.severity === 'high' ? 'bg-red-50' : ''}`}>
                    <td className="p-3 font-medium">{v.agentId}</td>
                    <td className="p-3">{v.metadata?.ruleId}</td>
                    <td className="p-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        v.metadata?.severity === 'high' ? 'bg-red-100 text-red-800' : 
                        v.metadata?.severity === 'medium' ? 'bg-yellow-100 text-yellow-800' : 
                    ...
                    <td className="p-3 text-gray-700 truncate max-w-xs">
                      {v.metadata?.snippet ? (
                        ...
                        <span className="text-gray-400 italic">No snippet available</span>
                      )}
                    </td>
                    <td className="p-3 text-gray-500">{new Date(v.timestamp).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="text-center py-12 bg-gray-50">
            <p className="text-gray-500">✓ No ethics violations recorded</p>
            <p className="text-gray-400 text-sm mt-1">All agents are operating within ethical boundaries</p>
          </div>
        )}
      </section>

      {/* Agent Activity Section could be added here */}
    </div>
  );
} 